...
class RGBDDataset(Dataset):
    def __init__(self, root_rgb_dir, root_depth_dir, transform=None, indices=None):
    ...
    def load_data(self):
        for trial_range in ['0-30', '31-59', '60-79']:
            ...
            for subdir in os.listdir(rgb_trial_path):
                if subdir.startswith("greyscale"):  
    ...
    def __getitem__(self, idx):
        ...
        try:
            rgb_image = Image.open(sample['rgb_img_path']).convert('L')
            depth_image = Image.open(sample['depth_img_path']).convert('L')

            image = np.stack((np.array(rgb_image), np.array(depth_image)), axis=2)
